...
- [4. Updated Requirements](#4-updated-requirements)
  - [4.1 Order Reservation Functionality](#41-order-reservation-functionality)
  - [4.2 Dish recommendation and ranking module](#42-dish-recommendation-and-ranking-module)
...
- [5. Updated Snapshots of the System's User Interface](#5-updated-snapshots-of-the-systems-user-interface)
  ...
  - [5.4 Campus Card Recharge](#54-campus-card-recharge)
...
**Detailed System Analysis**: Given the complexity of interactions across different systems, we have selected the Daily Life Services system for detailed analysis as a representative case study. This system provides in-depth insights into interaction patterns and design principles. It encompasses multiple aspects including dining services, dormitory management, campus card services, and other areas, featuring representative user interaction paths and business processes.
...
<p align="center">
  <img src="diagrams/ad_Presentation_Layer.svg" alt="Presentation Layer" style="width:50%; max-width:800px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/ad_safety_layer.svg" alt="Security Layer" style="width:50%; max-width:800px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/ad_business_layer.svg" alt="Business Layer" style="width:50%; max-width:800px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/ad_data_access.svg" alt="Data Access Layer" style="width:50%; max-width:800px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/ad_Infrastructure.svg" alt="Infrastructure Services Layer" style="width:50%; max-width:800px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/ad_data_storage.svg" alt="Data Storage Layer" style="width:50%; max-width:800px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/FeedBackInteraction.svg" alt="allscopes" title="scope" style="display:block; margin:0 auto; width:70%; max-width:500px; height:auto;"/>
</p>
...
### 4.1 Order Reservation Functionality
Through analysis of the current meal ordering system, we found that the existing system only supports immediate ordering (Browse Menu → Select Dishes → Place Order → Make Payment). In practice, students frequently encounter issues such as excessive waiting times during peak dining hours , popular dishes being sold out, and difficulty in planning meals in advance. To address these problems, we have simultaneously upgraded the reservation function, allowing students to make meal reservations in advance, select specific ordering times, and manage their reservation information, while optimizing the use case diagram design.
What's more, the original diagram had an overly complex structure due to too many use cases and confusing extend relationships. The current version simplifies the design by removing redundant use cases , retaining only core functionalities, and clearly distinguishing the roles of Students, Merchants. This makes the structure more concise and clear, improving readability and maintainability while better reflecting the system's fundamental features.
...
**USE CASE: Food Reservation Management**
| Item | Content |
|------|---------|
| **Use Case ID** | UC-001 |
...
### 4.2 Dish recommendation and ranking module

<p align="center">
  <img src="diagrams/2-UCD.svg" alt="Dish Recommendation and Ranking Use Case Diagram" style="width:50%; max-width:800px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/FeedbackUseCase.svg" alt="allscopes" title="scope" style="display:block; margin:0 auto; width:70%; max-width:500px; height:auto;"/>
</p>
...
<p align="center">
  <img src="diagrams/CampusCardUseCase.svg" alt="Campus Card Recharge Use Case Diagram" style="width:35%; max-width:800px; height:auto;"/>
</p>
...
<div style="display:flex; gap:10px; justify-content:center; flex-wrap:nowrap; overflow-x:auto;"> 
  <img src="diagrams/sp_mealorder0.png" alt="mealorder0" title="mealorder1" style="width:40%; max-width:320px; height:auto; display:block;" />
  <img src="diagrams/sp_mealorder1.png" alt="mealorder1" title="mealorder1" style="width:40%; max-width:320px; height:auto; display:block;" />
</div>  
...
<div style="display:flex; gap:10px; justify-content:center; flex-wrap:nowrap; overflow-x:auto;">
  <img src="diagrams/sp_mealorder3.png" alt="mealorder3" title="mealorder2" style="width:40%; max-width:320px; height:auto; display:block;" />
  <img src="diagrams/sp_mealorder4.png" alt="mealorder4" title="mealorder2" style="width:40%; max-width:320px; height:auto; display:block;" />
</div>  
...
This latest UI update shows the page for users to write and submit feedback on a specific dish, displaying basic information of the dish, including its name, rating, and sales, providing essential context as users compose their feedback. The feedback form offers a dual-mode input system, allowing users to express their opinions through both textual descriptions and images. 
...
This screenshot depicts a user interface designed for viewing the status and outcome of submitted feedback. The interface is structured into three distinct sections for optimal readability. Feedback entries are presented in a simple list format, including the dish name, restaurant name, and precise submission time. This provides essential context for the result that follows. The most prominent part of the screen is the "Result" section. This area delivers the final verdict on the user's submission. In this example, the result is positive: "Approved." This is followed by a direct message explaining the next steps, specifically that the restaurant will make contact to arrange the compensation method.
 
### 5.4 Campus Card Recharge
...
1. Real-time Data Consistency & Overbooking Prevention: Ensure cross-location consistency of inventory/availability; perform atomic deduction and validation for reservations/orders during peak periods (11:30–12:30, 17:30–18:30); maintain eventual consistency between successful payments and inventory deduction; implement exception compensation and reconciliation.  

2. High Concurrency & Peak Stability: Apply rate limiting / circuit breaking / concurrency control to core flows (order placement / payment / inventory check); enable elastic scaling and traffic smoothing; conduct load testing to meet SLO targets (e.g., 99.9% availability).  

3. Feedback Loop for Reviews: Use structured tags and automated ticket routing; deliver high-frequency/serious issues directly to merchants with progress tracking; provide categorized statistics and follow-up rectification visits.  

4. Multi-dimensional Rankings & Visualization: Introduce multi-dimensional scores such as taste trends, price-performance ratio, time-of-day popularity, and repurchase rate; offer interactive charts (line / radar charts, etc.); support filtering and comparison by restaurant, cuisine type, price range, etc.  

5. Intelligent & Personalized Recommendations: Build lightweight recommendation models (collaborative filtering / content-based recall + rules) based on votes, reviews, and consumption history; add related recommendations for new items (e.g., “You Might Like”); incorporate seasonality and context to improve click-through and conversion rates.  

6. Data Transparency & Trust: Display safety traceability info on dish pages (source, inspection reports, batch number); show authenticity labels for reviews (e.g., “Verified Diner”) and anti-fraud mechanisms; expose key metric definitions externally.
...
This thesis presents an early, influential architecture for transitioning large‑scale campus management from paper to integrated digital ecosystems. It systematically analyzes challenges in unifying heterogeneous campus services under a single digital framework, emphasizing database‑driven records, modular hot‑swappable components, workflow customization, and role‑based access control as foundational principles that directly inform the SmartCampus​ architecture. The study highlights consolidating fragmented legacy systems into a centralized identity authentication and personal portal to eliminate repeated logins and accelerate information access, aligning with SmartCampus’s goals of unified authentication and a consistent user experience.

The work also illustrates how to tailor digital campuses to specific institutions—here, a police training academy—while preserving a scalable, generalizable architecture. Subsystems are organized into trainee services, teaching management, logistics support, and performance evaluation, offering a useful precedent for SmartCampus’s decomposition across student affairs, academics, resource management, and administration. The described system was deployed and validated in practice, processing over 20,000​ data records with positive user feedback, providing empirical evidence for the feasibility of highly integrated platforms. Overall, it offers theoretical grounding and implementation insights that shape SmartCampus’s design philosophy, data integration strategy, and long‑term roadmap.
...
The team members collaborated actively, with tasks assigned in a balanced and well-defined manner. Together, they completed the project concept, planning, and system analysis, and each functional module was assigned appropriately.

| Members     | Works  | Percent |
| --------------------- | ----- |------- |
| Feng Juncai  2353924  | 1. Designed the class diagram and sequence interaction diagram for the meal order part.<br>2. Updated the use cases for meal order.<br>3. Updated the system snapshots for meal order and annotated the core functionalities.<br>4. Considered open issues.<br>5.Checking minor details in the documentation  |    27%    |
| Ji Peng  2351869      | 1. Write and improve the use case diagram, use case protocol and activity diagram of the campus card recharge part<br>2. Design the class/module structure of the campus card recharge part and draw the class diagram<br>3. Design a snapshot of the campus card recharge part according to the function<br>4. Collect and investigate references  |    23%    |
| Zhang Shikou  2353240 | 1.Analyze and refine the use case diagram, use case specifications, and interaction diagram for the Feedback subsystem.<br>2.Design the class/module structure for the Feedback subsystem and illustrate it with a class diagram.<br>3.Create system snapshots for the Feedback subsystem that demonstrate its key functionalities.<br>4.Collect and investigate relevant references and literature.  |    23%    |
| Yu Yilian  2352993    | 1. Complete the review section for student comments and dish recommendations.<br>2. Revise the corresponding use case diagram and use case descriptions.<br>3. Assist in checking minor details in the documentation.  |    27%    |
