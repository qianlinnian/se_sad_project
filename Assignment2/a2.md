...
| **Actors** | Student |
| **Preconditions** | The student has logged in to the SmartCampus system with valid credentials. |
...
| **Alternative Scenarios** | 4a. Database connection timeout → System shows "Unable to fetch balance, please try again later."<br>4b. Session expired → System prompts "Please re-login to continue." |
| **Postconditions** | The student successfully views the current balance of the campus card. |
...
|-------|-------------|
| **Use Case ID** | UC010 |
| **Use Case Name** | Recharge Campus Card |
...
| **Preconditions** | The student has a valid and active campus card account and is logged into SmartCampus. |
...
| **Alternative Scenarios** | 4a. Payment fails → The system records the failed transaction and displays "Payment unsuccessful."<br>4b. Payment timeout → The system retries once; if still failed, marks as pending.<br>5a. Student cancels before payment confirmation → The transaction is aborted, and no balance update occurs. |
| **Postconditions** | The campus card balance increases by the recharge amount, and the transaction is recorded successfully. |
...
|-------|-------------|
| **Use Case ID** | UC011 |
| **Use Case Name** | Confirm Payment Result |
...
| **Preconditions** | A recharge request has been initiated (UC02). |
...
| **Alternative Scenarios** | 1a. Invalid signature → The system rejects the callback and flags as suspicious.<br>2a. No callback received → The system periodically queries the payment gateway for status. |
| **Postconditions** | The system successfully verifies the payment outcome and updates transaction records. |
...
|-------|-------------|
| **Use Case ID** | UC012 |
| **Use Case Name** | Update Balance Record |
...
| **Preconditions** | The payment confirmation (UC03) has been successfully completed. |
...
| **Alternative Scenarios** | 3a. Network failure between SmartCampus and Financial System → System retries later and logs the issue.<br>4a. Data mismatch error → System rolls back the update and alerts the admin. |
| **Postconditions** | The balance data in both systems are synchronized successfully, and the student's balance is up to date. |
...
|-------|-------------|
| **Use Case ID** | UC013 |
| **Use Case Name** | Send Recharge Notification |
...
| **Preconditions** | Recharge transaction has been successfully processed. |
...
| **Alternative Scenarios** | 3a. Network delay or service down → Notification is queued and retried later.<br>4a. Student app offline → Notification will be resent when the app reconnects. |
| **Postconditions** | The student receives confirmation of the recharge completion. |
...
|-------|-------------|
| **Use Case ID** | UC014 |
| **Use Case Name** | Generate Transaction Log |
...
| **Preconditions** | A recharge operation has been successfully completed. |
...
| **Alternative Scenarios** | 2a. Log writing failure → System retries and reports error to system admin.<br>2b. Disk space full → System triggers cleanup of old logs. |
| **Postconditions** | The recharge transaction is fully recorded in the log system for future reference. |
...
The interface update includes adding a shopping cart section to the meal selection interface and providing a more detailed display of order contents.
...
#### 5.2.4 Merchant New Dish Submission Review Interface
...
#### 5.2.5 Admin Dish Approval List Interface
