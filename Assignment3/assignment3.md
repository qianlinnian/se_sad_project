...
As detailed in Sections 2.1 and 2.2 of Assignment 2, we adopted a microservices architecture that decouples the system into independently deployable services (e.g., order service, top-up service, voting service). Each service can be scaled horizontally based on load, using Kubernetes to manage automatic pod replication, with service discovery handled by Spring Cloud Kubernetes—a Spring Cloud extension that integrates directly with the Kubernetes API to discover services via native Kubernetes resources such as Services and Endpoints, eliminating the need for external registries like Netflix Eureka. This design directly addresses Assignment 2's requirement that "the system must be capable of scaling to support growth in user numbers and transaction volume."
...
- Zhang Shi Kou（2353240）:This project shifted my focus from just “making the system work” to thinking more systematically about *why* it is designed that way. By working on the feedback service subsystem, security mechanism, and the performance/maintainability sections, I started to see how API design, non-functional requirements and architecture all fit together rather than being separate tasks. Writing about layered security, caching, microservices and monitoring made me think in terms of trade-offs and constraints, not just listing technologies. Summarizing open issues in the model also reminded me that a design is always evolving and that clearly stating its limits is part of good engineering. 
- Yu Yilian (2352993):During the development of the dish recommendation and leaderboard subsystem, several issues emerged, including an overly simplistic and non-personalized recommendation algorithm, poor leaderboard performance due to the lack of caching, monotonous UI presentation, and limited functional dimensions. Additionally, the system suffered from architectural flaws such as tight coupling of algorithms and poor scalability. Future improvements should incorporate user behavior data and diverse recommendation models, adopt the strategy pattern to decouple algorithms, and enrich leaderboard categories and visual presentation of recommendations to comprehensively enhance system accuracy, performance, and user experience.
